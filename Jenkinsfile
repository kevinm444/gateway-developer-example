    ...
    stages {
        ...
        stage('Testing Docker image') {
            steps {
                script {
                    def jobDir = pwd()
                    env.GATEWAY_CONTAINER_ID = sh script: "docker run --env ACCEPT_LICENSE=true --env EXTRA_JAVA_ARGS=-Dcom.l7tech.bootstrap.env.license.enable=true --env ENV.PROPERTY.gateway.template=docker --env ENV.CONTEXT_PROPERTY.env-configuration.example=example --env ENV.CONTEXT_VARIABLE_PROPERTY.env-configuration.example="example value"--env ENV.PASSWORD.password=secret --env SSG_LICENSE=\$(cat $jobDir/../../license.xml | gzip | base64 --wrap=0) -p 8080 -p 8443 -d ${env.NEW_IMAGE_NAME}:${env.NEW_IMAGE_TAG} | cut -c 1-12", returnStdout: true
